<template>
  <div id="container" v-if="goodsDetail[0]">
          ...
          <div class="small">
            <img :class="select?'active':null" :src="goodsDetail[0].image_url" class="little" />
            <img class="hov" :src="goodsDetail[0].image_url" />
          </div>
				</div>
				<div class="pro_meg">
      ...
      <div class="pro_detail">
        <img :src="goodsDetail[0].details" style="width:100%"/>
      </div>
...
<script>
  ...
  export default {
    ...
    created(){
      //this.$route.params.id获取路由中的id参数
      this.currentGoodsId = Number(this.$route.params.id);
      //this.$store.dispatch('方法名'，值)【储存】，dispatch触发异步操作向后台发送请求获取数据
    ...
    methods:{
      ...
      async dealWithCellBtnClick(goods){
        ...
        if(this.userInfo.user_name){
          //await 也是一个修饰符，只能放在async定义的函数内。可以理解为等待。把这个非promise的东西当做await表达式的结果。
